...
const DATA_FILE_NAME = "plsort.json";
...
const SOURCE_LOGOS = {
  epic: { path: "assets/epic.png", label: "Epic" },
  gog: { path: "assets/gog.png", label: "GOG" },
  "에뮬": { path: "assets/emul.png", label: "에뮬" },
  emul: { path: "assets/emul.png", label: "에뮬" },
  emulator: { path: "assets/emul.png", label: "에뮬" },
  "drm-free": { path: "assets/drm-free.png", label: "DRM-free" },
  "drm free": { path: "assets/drm-free.png", label: "DRM-free" },
  "두기런처": { path: "assets/dugi.png", label: "두기런처" },
  dugi: { path: "assets/dugi.png", label: "두기런처" },
  "폰겜": { path: "assets/phone.png", label: "폰겜" },
  phone: { path: "assets/phone.png", label: "폰겜" },
  mobile: { path: "assets/phone.png", label: "폰겜" }
};
...
const DROPBOX_REMOTE_KEY = `playnite-sorter:dropbox-remote:${DATA_FILE_NAME}`;
...
function renderGameCover(row, indexes, title) {
  ...
  if (steamAppId) {
    ...
    elements.gameCover.setAttribute("aria-label", `${title} Steam 헤더 이미지`);
    elements.gameCover.classList.add("has-image");
    elements.gameCover.classList.remove("source-label", "source-logo-card");
    return;
  }

  const sourceLogo = getSourceLogo(sourceLabel);
  elements.gameCover.style.removeProperty("--game-image");
  elements.gameCover.classList.remove("has-image");

  if (sourceLogo) {
    const sourceImage = document.createElement("img");
    sourceImage.className = "source-logo";
    sourceImage.src = sourceLogo.path;
    sourceImage.alt = `${sourceLogo.label} 로고`;
    elements.gameCover.replaceChildren(sourceImage);
    elements.gameCover.setAttribute("aria-label", `${sourceLogo.label} 로고`);
    elements.gameCover.classList.remove("source-label");
    elements.gameCover.classList.add("source-logo-card");
    return;
  }

  elements.gameCover.textContent = sourceLabel;
  elements.gameCover.setAttribute("aria-label", sourceLabel);
  elements.gameCover.classList.remove("source-logo-card");
  elements.gameCover.classList.add("source-label");
}

function getSourceLogo(source) {
  return SOURCE_LOGOS[String(source).trim().toLowerCase()] || null;
}
...
function renderDropboxControls() {
  ...
  elements.dropboxConnectButton.textContent = connected ? "다시 연결" : "연결";
  ...
  if (!dropboxStatusMessage) {
    ...
    elements.dropboxStatus.title = dropboxStatusMessage;
  }
}
...
function setDropboxStatus(message) {
  ...
  elements.dropboxStatus.title = message;
}
