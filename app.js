...
async function loadHltbData() {
  ...
  try {
    ...
    if (EMBEDDED_HLTB_TSV.trim()) {
      hltbByGameId = parseHltbText(EMBEDDED_HLTB_TSV, "embedded-htlb.tsv");
      hltbLoadState = "ready";
      renderCurrentGame();
      return;
    }
...
function exportCsv() {
  ...
  showExportNotice("익스포트 완료. allgames-reviewed.csv를 내문서(Documents) 폴더에 넣고, 우측의 명령어 복사 버튼을 누르세요.");
